...
form.addEventListener('submit', async (event) => {
  ...
  if (provider !== 'none' && !username) {
    setStatus('Zadejte uživatelské jméno k úctu.');
    handleInput.focus();
    return;
  }
...
clearBtn.addEventListener('click', () => {
  ...
  const confirmed = window.confirm('Opravdu chcete smazat seznam registrovaných hrácu?');
  if (!confirmed) {
    return;
  }
...
tournamentCreateBtn.addEventListener('click', () => {
  ...
  if (!isRoundComplete(currentRound)) {
    setTournamentStatus('Nejdríve zadejte všechny výsledky tohoto kola.');
    return;
  }
...
function buildStatusMessage(entry) {
  if (entry.status === 'ok') {
    ...
    return `Rating${original} byl nacten. Prepoctený rating: ${entry.ratingFinal}.`;
  }
  ...
  for (const player of players) {
    ...
    saveBtn.addEventListener('click', () => {
      ...
      if (!Number.isFinite(nextRating) || nextRating < 0) {
        setStatus('Rating musí být nezáporné císlo.');
        return;
      }
...
function getPlayerNameById(id) {
  ...
  return player ? player.name : 'neznámý hrác';
}
...
function exportRoundPdf(round) {
  ...
  addPdfTable(doc, ['#', 'Bílý', 'Cerný', 'Výsledek'], rows, 90);
...
function exportTournamentPdf() {
  ...
  doc.setFontSize(18);
  doc.text('Turnaj Koruna - Výsledky', 40, 52);
  doc.setFontSize(11);
  doc.setTextColor(90);
  ...
  tournament.rounds.forEach((round) => {
    ...
    y = addPdfTable(doc, ['#', 'Bílý', 'Cerný', 'Výsledek'], buildRoundRows(round), y);
  });
...
function getSwissFailureMessage(result, allowRepeat) {
  if (result.reason === 'odd_group') {
    return 'Nelze vytvorit kolo: nekterá skupina zustala lichá (pravdepodobne už všichni meli volno).';
  }
...
function getPlayerResults(playerId, rounds) {
  ...
  rounds.forEach((round) => {
    round.pairings.forEach((pairing) => {
      ...
      results.push({
        round: round.round,
        opponent: opponent ? opponent.name : 'neznámý hrác',
        color,
        result
      });
    });
