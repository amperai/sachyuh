...
async function checkEngine() {
  try {
    const response = await fetch('/turnaj3/api/health', { cache: 'no-store' });
...
async function loadSharedState(showMessage = false) {
  const response = await fetch('/turnaj3/api/state', { cache: 'no-store' });
  const payload = await response.json();
...
async function persistState(message = 'Uloženo.') {
  ...
  const response = await fetch('/turnaj3/api/state', {
    method: 'POST',
    headers: {
...
async function generatePairings() {
  ...
  try {
    ...
    const response = await fetch(isRoundRobin ? '/turnaj3/api/round-robin' : '/turnaj3/api/pairings', {
      method: 'POST',
