...
test('Švıcar: bez opakování defaultne', () => {
...
test('Švıcar: opakování povoleno, kdy je potreba', () => {
  const players = [makePlayer('a', 1600), makePlayer('b', 1500)];
  const rounds = [makeRound(1, [
    { whiteId: 'a', blackId: 'b', result: '1-0' }
  ])];
...
test('Švıcar: bye jde nejníe postavenému bez bye', () => {
  const players = [
    makePlayer('a', 2000),
    makePlayer('b', 1900),
    makePlayer('c', 1800),
...
test('Švıcar: bye nesmí bıt dvakrát', () => {
  const players = [
    makePlayer('a', 1500),
    makePlayer('b', 1400),
    makePlayer('c', 1300)
...
test('Švıcar: barvy se neopakují, pokud je to moné', () => {
  const players = [
    makePlayer('a', 2000),
    makePlayer('b', 1900),
    makePlayer('c', 1800),
  ...
  result.pairings.forEach((pairing) => {
    ...
    assert(lastColors[pairing.whiteId] !== 'white', 'Bílı opakuje barvu.');
    assert(lastColors[pairing.blackId] !== 'black', 'Cernı opakuje barvu.');
  });
});
